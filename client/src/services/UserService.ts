...
class UserService {
    ...
    async getUserById(id: string) {
        return (await api.get<UserDetail>(this.ROUTE_PREFIX + `/by-id/${id}`)).data
    }
    async getUserByEmail(email: string) {
        return (await api.get<UserDetail>(this.ROUTE_PREFIX + `/by-email/${email}`)).data
    }
    async getUserByUsername(username: string) {
        return (await api.get<UserDetail>(this.ROUTE_PREFIX + `/by-username/${username}`)).data
    }
